...
export class CapturasList {
  ...
  cols = ['armadilha', 'data', 'status', 'situacao',  'aedes', 'culex', 'outras','total'];
  ...
  constructor(private router: Router) {
    ...
    this.router.events
      ...
      .subscribe(() => {
        if (this.router.url.includes('/workspace/mosquitos/capturas')) {
          this.loadData();
        }
      });
  }
  ...
  open(c: Captura, e?: Event) {
    e?.stopPropagation();
    this.router.navigate(['/workspace/mosquitos/capturas', c.id]);
  }
  ...
  statusColor(status: Captura['status']): string {
    return status === 'ATIVA' ? '3a9c00' : 'ff2e2e';
  }
  ...
  create() {
    this.router.navigate(['/workspace/mosquitos/capturas/new']);
  }
