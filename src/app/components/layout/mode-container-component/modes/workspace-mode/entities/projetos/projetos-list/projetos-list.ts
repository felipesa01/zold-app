...
export class ProjetosList {
  ...
  cols = ['nome', 'responsavel', 'cidade', 'uf', 'status', 'acoes'];
  ...
  constructor() {
    ...
    this.router.events
      ...
      .subscribe(() => {
        if (this.router.url.includes('/workspace/entities/projetos')) {
          this.loadData();
        }
      });
  }
  ...
  create() {
    this.router.navigate(['/workspace/entities/projetos/new']);
  }

  open(p: Projeto, e?: Event) {
    e?.stopPropagation();
    this.router.navigate(['/workspace/entities/projetos', p.id]);
  }
